...
function cacheStudentEnrolments(applications) {
...
function mergeCachedStudentEnrolment(application) {
...
export async function createStudentEnrolment(data) {
    ...
    const application = await parseApiResponse(response, "Unable to submit enrolment.");
...
export async function getStudentEnrolments() {
    ...
    const applications = await parseApiResponse(response, "Unable to load enrolments.");
    cacheStudentEnrolments(applications);
    return applications;
}
export async function getTeacherEnrolments() {
    const response = await fetch(`${API_BASE_URL}/teacher/enrolments`, {
        headers: getBearerHeaders(teacherSessionKey),
    });
    return parseApiResponse(response, "Unable to load enrolment requests.");
}
export async function updateTeacherEnrolmentStatus(applicationId, status) {
    ...
    return parseApiResponse(response, "Unable to update enrolment request.");
}
