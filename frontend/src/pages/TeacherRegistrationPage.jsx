...
const inputClass = "min-h-14 w-full rounded-lg border border-white/10 bg-black/25 px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-white/35 focus:border-champagne/60 focus:ring-2 focus:ring-champagne/20 sm:text-base";
const labelClass = "text-xs font-black uppercase tracking-[0.18em] text-champagne";
const sectionClass = "rounded-[1.5rem] border border-[#5c2520]/85 bg-[#180607]/88 p-6 shadow-[0_26px_80px_rgba(0,0,0,0.24)] backdrop-blur-xl sm:p-8";
...
export function TeacherRegistrationPage() {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            setSelectedFileName("");
            await showSuccessAlert("Application Submitted", `Application submitted for ${teacher.fullName}. You can login after admin approval.`);
        }
        catch (registrationError) {
            await showErrorAlert("Application Failed", registrationError instanceof Error
    ...
    return (<div className="min-h-screen overflow-hidden bg-[#120405] text-white">
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-[17rem_1fr] lg:items-start">
                <div className="grid gap-3">
                  ...
                  <label className="group grid min-h-[15rem] cursor-pointer place-items-center rounded-2xl border border-dashed border-white/20 bg-black/25 p-5 text-center transition hover:border-champagne/60 hover:bg-champagne/10">
                    <span className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-full border border-white/15 bg-[#100405] shadow-[0_0_32px_rgba(244,199,107,0.14)]">
                      {avatarPreviewDataUrl ? (<img src={avatarPreviewDataUrl} alt="" className="h-full w-full object-cover"/>) : (<ImagePlus className="text-champagne" size={36}/>)}
                    </span>
                  ...
                  {avatarFileName && (<button type="button" onClick={clearAvatarAttachment} className="inline-flex w-fit items-center gap-2 text-sm font-black text-white/60 transition hover:text-champagne">
                      <X size={16}/>
                      Remove image
                    </button>)}
                </div>
                ...
                <div className="grid content-start gap-5 sm:grid-cols-2">
                  ...
                  <label className="grid gap-3">
                    ...
                    <input className={inputClass} name="email" type="email" placeholder="alex.rivers@sankalana.com" required/>
                  </label>
            ...
            <section className={sectionClass}>
              ...
              <div className="mt-6 grid gap-5 lg:grid-cols-3">
                <label className="grid gap-3">
                  ...
                  <select className={`${inputClass} cursor-pointer`} name="danceStyles" defaultValue="" required>
                    ...
                    {danceStyleOptions.map((style) => (<option key={style} value={style} className="bg-[#180607] text-white">
                        {style}
                      </option>))}
                  </select>
                </label>
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_24rem]">
                <label className="grid gap-3">
                  ...
                  <textarea className={`${inputClass} min-h-[17rem] resize-none leading-7`} name="biography" placeholder="Tell us about your journey, teaching philosophy, and what makes your classes unique..." required/>
                </label>
                ...
                <div className="grid content-start gap-6">
                  <div className="grid gap-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {teachingDays.map((day) => (<button key={day} type="button" onClick={() => toggleTeachingDay(day)} className={cn("min-h-11 min-w-14 rounded-full border px-4 text-sm font-black transition", availableDays.includes(day)
            ...
            <section className={sectionClass}>
              ...
              <div className="mt-6 grid gap-5 lg:grid-cols-3">
                ...
                <label className="grid gap-3">
                  ...
                  <input className={inputClass} name="confirmPassword" type="password" minLength={6} placeholder="••••••" required/>
                </label>
              </div>
            </section>

            <div className="flex justify-end">
              <button type="submit" disabled={isSubmitting} className="inline-flex min-h-12 w-full items-center justify-center gap-3 rounded-lg bg-champagne px-7 py-3 text-sm font-extrabold text-ink transition duration-200 hover:-translate-y-0.5 hover:bg-[#ffd887] hover:shadow-[0_0_22px_rgba(244,199,107,0.28)] disabled:cursor-not-allowed disabled:opacity-70 disabled:hover:translate-y-0 sm:w-auto">
