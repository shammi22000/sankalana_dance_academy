...
const registrationOptions = [
    {
        ...
        description: "Join classes, submit enrolment for our programs, and track your choreographic progress through your personal dashboard.",
    ...
    },
    {
        ...
        description: "Manage your curriculum, track student attendance, and organize performances after profile approval.",
      ...
      <main className="relative min-h-[calc(100svh-5rem)] px-4 py-20 sm:px-6 lg:px-8">
        ...
        <section className="relative z-10 mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-2">
            ...
            return (<article key={option.role} className="group relative overflow-hidden rounded-[1.7rem] border border-[#5c2520] bg-[#180607]/82 p-8 text-center shadow-[0_26px_80px_rgba(0,0,0,0.25)] backdrop-blur-xl transition duration-500 hover:-translate-y-1 hover:border-champagne/50 sm:p-10">
                  <div className={cn("absolute -right-24 -top-24 h-64 w-64 rounded-full blur-3xl transition group-hover:scale-110", isTeacher ? "bg-cyanGlow/10" : "bg-champagne/10")}/>
                  <div className="relative z-10">
                    <span className={cn("mx-auto inline-flex h-20 w-20 items-center justify-center rounded-full", isTeacher ? "bg-cyanGlow/10 text-cyanGlow" : "bg-champagne text-[#170706]")}>
                    ...
                    <ul className="mx-auto mt-10 grid max-w-md gap-5 text-left">
                      {option.benefits.map((benefit) => (<li key={benefit} className="flex items-center gap-4 text-sm font-black tracking-[0.08em] text-white/80">
                          {isTeacher ? (<ShieldCheck className="shrink-0 text-cyanGlow" size={22}/>) : (<BadgeCheck className="shrink-0 text-[#e7a6ff]" size={22}/>)}
                          {benefit}
                        </li>))}
                    </ul>

                    <Link to={isTeacher ? "/teacher-register" : "/student-register"} className={cn("mt-10 inline-flex min-h-11 w-full items-center justify-center gap-3 rounded-lg px-6 py-3 text-sm font-extrabold transition duration-300", isTeacher
