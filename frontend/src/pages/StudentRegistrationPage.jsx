...
export function StudentRegistrationPage() {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            form.reset();
            await showSuccessAlert("Registration Submitted", `Registration saved for ${student.fullName}. You can login after admin approval.`);
        }
        catch (registrationError) {
            await showErrorAlert("Registration Failed", registrationError instanceof Error
          ...
          <form className="mt-10 grid gap-6" onSubmit={handleSubmit}>
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <label className="grid gap-3">
                ...
                <input className={inputClass} name="password" type="password" placeholder="••••••" minLength={6} required/>
              </label>
              ...
              <label className="grid gap-3">
                ...
                <input className={inputClass} name="confirmPassword" type="password" placeholder="••••••" minLength={6} required/>
              </label>
            </div>
            ...
            <button type="submit" disabled={isSubmitting} className="mt-4 inline-flex min-h-14 items-center justify-center gap-3 rounded-lg bg-champagne px-6 py-3 text-sm font-extrabold text-ink transition duration-200 hover:-translate-y-0.5 hover:bg-[#ffd887] hover:shadow-[0_0_22px_rgba(244,199,107,0.28)] disabled:cursor-not-allowed disabled:opacity-70">
            ...
            <div className="flex flex-col gap-4 border-t border-white/10 pt-8 text-sm font-semibold text-white/80 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link to="/teacher-register" className="inline-flex items-center gap-2 font-black text-champagne transition hover:text-white">
