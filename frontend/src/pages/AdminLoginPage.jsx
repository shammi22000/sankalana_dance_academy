...
export function AdminLoginPage() {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            await showErrorAlert("Login Failed", loginError instanceof Error ? loginError.message : "Invalid admin username or password.");
        }
        finally {
    ...
    return (<div className="min-h-screen bg-black text-white">
      ...
      <main className="relative min-h-[calc(100svh-5rem)] overflow-hidden px-4 py-16 sm:px-6 lg:px-8">
        <img src={danceImages.heroCarousel[2].src} alt="" className="absolute inset-0 h-full w-full object-cover opacity-20 blur-[1px] saturate-125"/>
              ...
              <form className="mt-10 grid gap-6" onSubmit={handleSubmit}>
                <label className="grid gap-3">
                  ...
                  <span className="flex min-h-14 items-center gap-4 rounded-xl border border-white/12 bg-black/30 px-5 transition focus-within:border-cyanGlow/55 focus-within:ring-2 focus-within:ring-cyanGlow/15">
                    ...
                    <input name="username" placeholder="admin" className="w-full bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/35 sm:text-base" autoComplete="username" disabled={isSubmitting} required/>
                  </span>
                </label>
                ...
                <label className="grid gap-3">
                  ...
                  <span className="flex min-h-14 items-center gap-4 rounded-xl border border-white/12 bg-black/30 px-5 transition focus-within:border-cyanGlow/55 focus-within:ring-2 focus-within:ring-cyanGlow/15">
                    ...
                    <input name="password" type="password" placeholder="admin" className="w-full bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/35 sm:text-base" autoComplete="current-password" disabled={isSubmitting} required/>
                  </span>
                </label>

                <button type="submit" disabled={isSubmitting} className="mt-3 inline-flex min-h-14 items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-[#bb26ff] to-[#e026b4] px-6 py-3 text-lg font-black text-white shadow-[0_18px_55px_rgba(217,28,255,0.36)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-70 disabled:hover:translate-y-0">
