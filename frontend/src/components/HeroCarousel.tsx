...
export function HeroCarousel({ images, className }: HeroCarouselProps) {
  ...
  return (
    <div className={cn("absolute inset-0 overflow-hidden bg-[#09070c]", className)}>
      {images.map((image, index) => (
        <img
          ...
          alt={image.alt}
          className={cn(
            "absolute inset-0 h-full w-full object-cover object-center saturate-125 transition-all duration-1000 ease-out motion-reduce:transition-none",
            index === activeIndex
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="mt-3 grid grid-cols-[1fr_auto] items-center gap-4">
              <div className="h-1.5 overflow-hidden rounded-full bg-white/15">
                <div
                  ...
                  className={cn(
                    "h-full rounded-full bg-gradient-to-r from-champagne via-orchid to-cyanGlow",
                    isPlaying && "animate-[carouselProgress_4600ms_linear_forwards]",
                  )}
                />
              </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-white/20 bg-black/35 text-white backdrop-blur transition hover:bg-white/15"
