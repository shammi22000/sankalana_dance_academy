      ...
      <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[0.95fr_1fr]">
        <div className="grid grid-cols-2 gap-5">
          {storyImages.map((image, index) => (
            <div
              key={image}
              className="group overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-[0_24px_60px_rgba(0,0,0,0.25)]"
            >
              <img
                src={image}
                alt={`Dance academy moment ${index + 1}`}
                loading="lazy"
                className="aspect-square h-full w-full object-cover opacity-85 transition duration-500 group-hover:scale-105 group-hover:opacity-100"
              />
            </div>
          ))}
        </div>
