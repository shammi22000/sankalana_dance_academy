...
export function AuthLoginPage({ roleLabel, subtitle, statusMessage, onSubmit }) {
    ...
    return (<div className="min-h-screen bg-[#120405] text-white">
      <PageHeader ctaLabel="Register" ctaTo={registrationPath}/>

      <main className="relative min-h-[calc(100svh-5rem)] overflow-hidden">
        <img src={roleLabel === "Student" ? danceImages.heroCarousel[1].src : danceImages.heroCarousel[2].src} alt="" className="absolute inset-0 h-full w-full object-cover opacity-30 blur-[1px] saturate-125"/>
            ...
            <form className="mt-10 grid gap-6" onSubmit={handleSubmit}>
              <label className="grid gap-3">
                ...
                <span className="flex min-h-14 items-center gap-4 rounded-lg border border-white/10 bg-[#100404]/90 px-5 transition focus-within:border-champagne/60 focus-within:ring-2 focus-within:ring-champagne/20">
                  ...
                  <input name="identity" placeholder="Email or Username" className="w-full bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/35 sm:text-base" autoComplete="username" disabled={status === "submitting"} required/>
                </span>
              </label>
              ...
              <label className="grid gap-3">
                <span className="flex items-center justify-between gap-4">
                  ...
                  <Link to="/#contact" className="text-xs font-black uppercase tracking-[0.14em] text-cyanGlow transition hover:text-white">
                    Forgot Password?
                  </Link>
                </span>
                <span className="flex min-h-14 items-center gap-4 rounded-lg border border-white/10 bg-[#100404]/90 px-5 transition focus-within:border-champagne/60 focus-within:ring-2 focus-within:ring-champagne/20">
                  <LockKeyhole className="shrink-0 text-white/55" size={25}/>
                  <input name="password" type={showPassword ? "text" : "password"} placeholder="••••••••••••" className="w-full bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/35 sm:text-base" autoComplete="current-password" disabled={status === "submitting"} required/>
                  <button type="button" onClick={() => setShowPassword((visible) => !visible)} className="text-white/55 transition hover:text-white" aria-label={showPassword ? "Hide password" : "Show password"}>
                    {showPassword ? <EyeOff size={24}/> : <Eye size={24}/>}
                  </button>
                </span>
              </label>

              <button type="submit" disabled={status === "submitting"} className="mt-2 inline-flex min-h-14 items-center justify-center gap-3 rounded-lg bg-champagne px-6 py-3 text-sm font-extrabold text-ink transition duration-200 hover:-translate-y-0.5 hover:bg-[#ffd887] hover:shadow-[0_0_22px_rgba(244,199,107,0.28)] disabled:cursor-not-allowed disabled:opacity-70 disabled:hover:translate-y-0">
