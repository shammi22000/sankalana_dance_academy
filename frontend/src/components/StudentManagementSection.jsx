import { BadgeCheck, Cake, CalendarDays, ChevronDown, ChevronLeft, ChevronRight, Clock3, IdCard, LockKeyhole, Mail, Phone, Search, ShieldCheck, UserRound, UserRoundCheck, X, } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { createPortal } from "react-dom";
import { getStudentRegistrations, updateStudentApprovalStatus, updateStudentPassword, updateStudentRegistrationProfile, } from "../services/adminRegistrationService";
...
const studentGenderOptions = ["Female", "Male", "Other", "Prefer not to say"];
const profileInputClass = "min-h-12 w-full rounded-lg border border-white/10 bg-[#0b0310] px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-white/35 focus:border-[#f0b7ff]/60 focus:ring-2 focus:ring-[#f0b7ff]/20";
...
export function StudentManagementSection() {
    ...
    useEffect(() => {
        ...
        getStudentRegistrations()
            ...
            .catch((error) => {
            if (!ignore) {
                void showErrorAlert("Unable to Load Students", error instanceof Error ? error.message : "Unable to load student registrations.");
            }
        })
            .finally(() => {
            if (!ignore) {
    ...
    const directoryStudents = useMemo(() => studentRegistrations.map(toDirectoryStudent), [studentRegistrations]);
    ...
    async function handleToggleStudent(student) {
        const nextStatus = student.status === "approved" ? "rejected" : "approved";
        setUpdatingStudentId(student.id);
        try {
            const updatedStudent = await updateStudentApprovalStatus(student.id, nextStatus);
            setStudentRegistrations((currentStudents) => currentStudents.map((currentStudent) => currentStudent.id === updatedStudent.id ? updatedStudent : currentStudent));
            setSelectedStudent((currentStudent) => currentStudent?.id === updatedStudent.id ? toDirectoryStudent(updatedStudent) : currentStudent);
            await showSuccessAlert(nextStatus === "approved" ? "Student Activated" : "Student Deactivated", `${updatedStudent.fullName} is now ${statusLabel(updatedStudent.approvalStatus)}.`);
        }
        catch (error) {
            await showErrorAlert("Update Failed", error instanceof Error ? error.message : "Unable to update student status.");
        }
        finally {
    ...
    return (<section className="relative z-10 mx-auto max-w-7xl">
      <div className="flex flex-col gap-6 border-b border-white/10 pb-6 xl:flex-row xl:items-center xl:justify-between">
        ...
        <label className="flex min-h-12 items-center gap-3 rounded-full border border-white/10 bg-[#1a0a20]/92 px-5 text-white/70 shadow-[0_18px_55px_rgba(0,0,0,0.22)] focus-within:border-orchid/45">
          ...
          <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="w-full bg-transparent text-sm font-bold text-white outline-none placeholder:text-white/35 sm:w-72" placeholder="Search students..."/>
        </label>
      </div>
      ...
      {isLoadingStudents ? (<div className="mt-8 rounded-[1.35rem] border border-white/[0.12] bg-white/[0.055] p-8 text-sm font-black text-white/65">
          Loading student registrations...
        </div>) : filteredStudents.length === 0 ? (<div className="mt-8 rounded-[1.35rem] border border-white/[0.12] bg-white/[0.055] p-8 text-sm font-black text-white/65">
          No students match the current filters.
        </div>) : (<div className="mt-8 grid gap-6 lg:grid-cols-3">
          {paginatedStudents.map((student) => (<StudentCard key={student.id} student={student} onToggle={() => void handleToggleStudent(student)} onView={() => setSelectedStudent(student)} isUpdating={updatingStudentId === student.id}/>))}
        </div>)}

      {shouldShowPagination && (<div className="mt-10 flex justify-center gap-3">
          <button type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={currentPage === 1} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/10 text-white/65 transition hover:border-cyanGlow/40 hover:text-cyanGlow disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:border-white/10 disabled:hover:text-white/65" aria-label="Previous students page">
            <ChevronLeft size={22}/>
          </button>
          {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (<button key={page} type="button" onClick={() => setCurrentPage(page)} className={`h-11 w-11 rounded-full border text-sm font-black transition ${page === currentPage
                    ...
                    : "border-white/10 text-white/58 hover:border-cyanGlow/35 hover:text-cyanGlow"}`} aria-label={`Students page ${page}`} aria-current={page === currentPage ? "page" : undefined}>
              {page}
            </button>))}
          <button type="button" onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))} disabled={currentPage === totalPages} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/10 text-white/65 transition hover:border-cyanGlow/40 hover:text-cyanGlow disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:border-white/10 disabled:hover:text-white/65" aria-label="Next students page">
            <ChevronRight size={22}/>
          </button>
        </div>)}

      {selectedStudent && (<StudentProfileModal student={selectedStudent} onSave={handleSaveStudentProfile} onClose={() => setSelectedStudent(null)}/>)}
    </section>);
}
function StudentCard({ student, onToggle, onView, isUpdating, }) {
    const isActive = student.status === "approved";
    const muted = student.status === "rejected";
    return (<article className={`min-h-[20rem] rounded-[1.35rem] border p-6 shadow-[0_24px_90px_rgba(0,0,0,0.26)] ${muted
      ...
      <div className="flex items-start justify-between gap-5">
        ...
        <button type="button" onClick={onToggle} disabled={isUpdating} className={`relative h-6 w-12 shrink-0 rounded-full transition disabled:cursor-not-allowed disabled:opacity-60 ${isActive ? "bg-orchid" : "bg-white/12"}`} aria-label={`${isActive ? "Deactivate" : "Activate"} ${student.name}`}>
          <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${isActive ? "left-7" : "left-1"}`}/>
        </button>
      </div>

      <div className="mt-7 flex flex-wrap gap-2">
        {[statusLabel(student.status), student.gender, `${student.age} Years`].map((badge) => (<span key={badge} className="rounded-md border border-white/10 bg-white/[0.055] px-3 py-2 text-xs font-black text-white/56">
            {badge}
          </span>))}
      </div>
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-5">
        ...
        <button type="button" onClick={onView} className="inline-flex items-center gap-2 text-sm font-black text-[#f0b7ff] transition hover:text-white">
          View Profile
...
function StudentProfileModal({ student, onSave, onClose, }) {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            await showErrorAlert("Profile Not Updated", error instanceof Error ? error.message : "Unable to update student profile.");
        }
        finally {
            setIsSaving(false);
        }
    ...
    async function handlePasswordSubmit(event) {
        ...
        try {
            ...
            await showErrorAlert("Password Not Updated", error instanceof Error ? error.message : "Unable to update student password.");
        }
        finally {
    ...
    return createPortal(<div className="fixed inset-0 z-[9999] overflow-y-auto bg-black px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-5xl rounded-[1.35rem] border border-white/12 bg-[#16051d] p-5 shadow-[0_32px_110px_rgba(0,0,0,0.55)] sm:p-7">
        <div className="flex items-start justify-between gap-5 border-b border-white/10 pb-6">
          ...
          <button type="button" onClick={onClose} disabled={isSaving} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/10 bg-[#211028] text-white/65 transition hover:border-[#f0b7ff]/45 hover:text-[#f0b7ff]" aria-label="Close student profile">
            <X size={22}/>
          </button>
        </div>
          ...
          <div className="grid gap-6">
            <form className="rounded-2xl border border-white/10 bg-[#211028] p-5 sm:p-6" onSubmit={handleSubmit}>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className={`inline-flex w-fit items-center gap-2 rounded-full border px-4 py-2 text-xs font-black uppercase tracking-[0.1em] ${isApproved
            ...
            : isRejected
              ...
              {isEditing ? (<div className="mt-6 grid gap-5 sm:grid-cols-2">
                  ...
                  <label className="grid gap-3 sm:col-span-2">
                    ...
                    <select className={`${profileInputClass} cursor-pointer`} name="gender" defaultValue={student.gender} required>
                      {studentGenderOptions.map((gender) => (<option key={gender} value={gender} className="bg-[#140616] text-white">
                          {gender}
                        </option>))}
                    </select>
                  </label>
              ...
              {isEditing && (<div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
                  <button type="button" onClick={() => setIsEditing(false)} disabled={isSaving} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/12 px-6 text-sm font-black text-white/70 transition hover:border-white/30 hover:text-white disabled:cursor-not-allowed disabled:opacity-60">
                    Cancel Edit
                  </button>
                  <button type="submit" disabled={isSaving} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[#f0b7ff] px-6 text-sm font-black text-[#17061d] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-70">
            ...
            <section className="rounded-2xl border border-white/10 bg-[#211028] p-5">
              ...
              {isPasswordEditing ? (<form className="mt-5 grid gap-4" onSubmit={handlePasswordSubmit}>
                  <label className="grid gap-2">
                    ...
                    <input className={profileInputClass} name="password" type="password" minLength={6} placeholder="New password" required/>
                  </label>
                  <label className="grid gap-2">
                    <span className={profileLabelClass}>Confirm Password</span>
                    <input className={profileInputClass} name="confirmPassword" type="password" minLength={6} placeholder="Confirm password" required/>
                  </label>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <button type="button" onClick={() => setIsPasswordEditing(false)} disabled={isUpdatingPassword} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/12 px-4 text-sm font-black text-white/70 transition hover:border-white/30 hover:text-white disabled:cursor-not-allowed disabled:opacity-60">
                      Cancel
                    </button>
                    <button type="submit" disabled={isUpdatingPassword} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-cyanGlow px-4 text-sm font-black text-[#061014] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-70">
                      {isUpdatingPassword ? "Updating..." : "Save Password"}
                    </button>
                  </div>
                </form>) : (<button type="button" onClick={() => setIsPasswordEditing(true)} disabled={isSaving} className="mt-5 inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-cyanGlow/35 px-5 text-sm font-black text-cyanGlow transition hover:bg-cyanGlow/10 disabled:cursor-not-allowed disabled:opacity-60">
                  Set New Password
                </button>)}
            </section>

            <div className="grid gap-3">
              {!isEditing && (<button type="button" onClick={() => setIsEditing(true)} disabled={isSaving} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[#f0b7ff] px-6 text-sm font-black text-[#17061d] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-70">
                  Edit Profile
                </button>)}
              <button type="button" onClick={onClose} disabled={isSaving} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/12 px-6 text-sm font-black text-white/70 transition hover:border-white/30 hover:text-white disabled:cursor-not-allowed disabled:opacity-60">
                Close Profile
              </button>
            </div>
          </aside>
...
function ProfileDetail({ icon: Icon, label, value, }) {
    return (<div className="rounded-2xl border border-white/10 bg-[#0b0310] p-4">
      <div className="flex items-center gap-3">
...
function toDirectoryStudent(student) {
    return {
        id: student.id,
