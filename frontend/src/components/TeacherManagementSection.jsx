import { ArrowRight, BadgeCheck, BriefcaseBusiness, CalendarDays, ChevronDown, ChevronLeft, ChevronRight, CloudUpload, FileText, ImagePlus, LockKeyhole, Mail, Phone, Search, ShieldCheck, UserPlus, UserRound, X, } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { createPortal } from "react-dom";
import { danceImages } from "../assets/danceImages";
import { createTeacherRegistration, getTeacherRegistrations, updateTeacherApplicationStatus, updateTeacherPassword, } from "../services/adminRegistrationService";
...
const modalInputClass = "min-h-12 w-full rounded-lg border border-white/10 bg-[#0b0310] px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-white/35 focus:border-[#f0b7ff]/60 focus:ring-2 focus:ring-[#f0b7ff]/20";
...
export function TeacherManagementSection() {
    ...
    useEffect(() => {
        ...
        getTeacherRegistrations()
            ...
            .catch((error) => {
            if (!ignore) {
                void showErrorAlert("Unable to Load Teachers", error instanceof Error ? error.message : "Unable to load teacher registrations.");
            }
        })
            .finally(() => {
            if (!ignore) {
    ...
    const directoryTeachers = useMemo(() => {
        const displayIds = new Map([...teacherRegistrations]
            .sort((first, second) => Date.parse(first.createdAt) - Date.parse(second.createdAt))
            .map((teacher, index) => [teacher.id, String(index + 1)]));
        return teacherRegistrations.map((teacher, index) => toDirectoryTeacher(teacher, index, displayIds.get(teacher.id) ?? String(index + 1)));
    ...
    async function handleToggleTeacher(teacher) {
        ...
        try {
            ...
            setTeacherRegistrations((currentTeachers) => currentTeachers.map((currentTeacher) => currentTeacher.id === updatedTeacher.id ? updatedTeacher : currentTeacher));
            await showSuccessAlert(nextStatus === "approved" ? "Teacher Activated" : "Teacher Deactivated", `${updatedTeacher.fullName} is now ${statusLabel(updatedTeacher.applicationStatus)}.`);
        }
        catch (error) {
            await showErrorAlert("Update Failed", error instanceof Error ? error.message : "Unable to update teacher status.");
        }
        finally {
    ...
    async function handleCreateTeacher(payload) {
        ...
        try {
            ...
            await showSuccessAlert("Teacher Added", `${createdTeacher.fullName} has been added and approved for login.`);
        }
        catch (error) {
            await showErrorAlert("Teacher Not Added", error instanceof Error ? error.message : "Unable to create teacher.");
        }
        finally {
            setIsCreatingTeacher(false);
        }
    }
    return (<section className="relative z-10 mx-auto max-w-7xl">
      <div className="flex flex-col gap-6 border-b border-white/10 pb-6 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <label className="flex min-h-12 items-center gap-3 rounded-full border border-white/10 bg-[#1a0a20]/92 px-5 text-white/70 shadow-[0_18px_55px_rgba(0,0,0,0.22)] focus-within:border-orchid/45">
            ...
            <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="w-full bg-transparent text-sm font-bold text-white outline-none placeholder:text-white/35 sm:w-64" placeholder="Search teachers..."/>
          </label>

          <button type="button" onClick={() => setIsAddTeacherOpen(true)} className="inline-flex min-h-12 items-center justify-center gap-3 rounded-full border border-orchid/35 bg-orchid/35 px-6 text-sm font-black text-[#f3c5ff] shadow-[0_18px_55px_rgba(217,28,255,0.22)] transition hover:bg-orchid/45">
            <UserPlus size={21}/>
            Add New Teacher
          </button>
        </div>
      ...
      <div className="mt-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-3">
          {filters.map((filter) => (<button key={filter.value} type="button" onClick={() => setActiveFilter(filter.value)} className={`min-h-9 rounded-full px-4 text-xs font-black transition ${activeFilter === filter.value
        ...
        <label className="flex w-fit items-center gap-3 text-sm font-black text-white/60">
          ...
          <span className="relative">
            <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="appearance-none bg-transparent py-2 pr-8 text-[#f0b7ff] outline-none">
      ...
      {isLoadingTeachers ? (<div className="mt-8 rounded-[1.35rem] border border-white/[0.12] bg-white/[0.055] p-8 text-sm font-black text-white/65">
          Loading teacher registrations...
        </div>) : filteredTeachers.length === 0 ? (<div className="mt-8 rounded-[1.35rem] border border-white/[0.12] bg-white/[0.055] p-8 text-sm font-black text-white/65">
          No teachers match the current filters.
        </div>) : (<div className="mt-8 grid gap-6 lg:grid-cols-3">
          {paginatedTeachers.map((teacher) => (<TeacherCard key={teacher.id} teacher={teacher} onToggle={() => void handleToggleTeacher(teacher)} onViewProfile={() => setSelectedTeacher(teacher)} isUpdating={updatingTeacherId === teacher.id}/>))}
        </div>)}

      {shouldShowPagination && (<div className="mt-10 flex justify-center gap-3">
          <button type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={currentPage === 1} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/10 text-white/65 transition hover:border-cyanGlow/40 hover:text-cyanGlow disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:border-white/10 disabled:hover:text-white/65" aria-label="Previous teachers page">
            <ChevronLeft size={22}/>
          </button>
          {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (<button key={page} type="button" onClick={() => setCurrentPage(page)} className={`h-11 w-11 rounded-full border text-sm font-black transition ${page === currentPage
                    ? "border-[#f0b7ff] bg-[#f0b7ff] text-[#17061d]"
                    : "border-white/10 text-white/58 hover:border-cyanGlow/35 hover:text-cyanGlow"}`} aria-label={`Teachers page ${page}`} aria-current={page === currentPage ? "page" : undefined}>
              {page}
            </button>))}
          <button type="button" onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))} disabled={currentPage === totalPages} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/10 text-white/65 transition hover:border-cyanGlow/40 hover:text-cyanGlow disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:border-white/10 disabled:hover:text-white/65" aria-label="Next teachers page">
            <ChevronRight size={22}/>
          </button>
        </div>)}

      {isAddTeacherOpen && (<AddTeacherModal isSubmitting={isCreatingTeacher} onClose={() => setIsAddTeacherOpen(false)} onSubmit={handleCreateTeacher}/>)}

      {selectedTeacher && (<TeacherProfileModal teacher={selectedTeacher} onClose={() => setSelectedTeacher(null)}/>)}
    </section>);
}
function toDirectoryTeacher(teacher, index, displayId) {
...
function TeacherCard({ teacher, onToggle, onViewProfile, isUpdating, }) {
    const isActive = teacher.status === "approved";
    const muted = teacher.status === "rejected";
    return (<article className={`min-h-[21rem] rounded-[1.35rem] border p-6 shadow-[0_24px_90px_rgba(0,0,0,0.26)] ${muted
            ? "border-white/[0.08] bg-[#0b0d0f]/86 text-white/52"
            : "border-white/[0.12] bg-[#17091d]/88 text-white"}`}>
      <div className="flex items-start justify-between gap-5">
        <div className="flex min-w-0 gap-4">
          <img src={teacher.avatar} alt="" className={`h-16 w-16 shrink-0 rounded-2xl border border-white/10 object-cover ${muted ? "grayscale" : ""}`}/>
        ...
        <button type="button" onClick={onToggle} disabled={isUpdating} className={`relative h-6 w-12 shrink-0 rounded-full transition disabled:cursor-not-allowed disabled:opacity-60 ${isActive ? "bg-orchid" : "bg-white/12"}`} aria-label={`${isActive ? "Deactivate" : "Activate"} ${teacher.name}`}>
          <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${isActive ? "left-7" : "left-1"}`}/>
        </button>
      </div>

      <div className="mt-7 flex flex-wrap gap-2">
        {teacher.badges.map((badge) => (<span key={badge} className="rounded-md border border-white/10 bg-white/[0.055] px-3 py-2 text-xs font-black text-white/56">
            {badge}
          </span>))}
      </div>
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-5">
        ...
        <button type="button" onClick={onViewProfile} className="inline-flex items-center gap-2 text-sm font-black text-[#f0b7ff] transition hover:text-white">
          View Profile
...
function TeacherProfileModal({ teacher, onClose, }) {
    ...
    async function handlePasswordSubmit(event) {
        ...
        try {
            ...
            await showErrorAlert("Password Not Updated", error instanceof Error ? error.message : "Unable to update teacher password.");
        }
        finally {
    ...
    return createPortal(<div className="fixed inset-0 z-[9999] overflow-y-auto bg-black px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-6xl rounded-[1.35rem] border border-white/12 p-5 shadow-[0_32px_110px_rgba(0,0,0,0.55)] sm:p-7" style={{ backgroundColor: modalBackgroundColor }}>
        <div className="flex items-start justify-between gap-4 border-b border-white/10 pb-5">
          ...
          <button type="button" onClick={onClose} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 text-white/65 transition hover:border-[#f0b7ff]/45 hover:text-[#f0b7ff]" aria-label="Close teacher profile">
            <X size={21}/>
          </button>
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[20rem_1fr]">
          <aside className="rounded-2xl border border-white/10 p-6 text-center" style={{ backgroundColor: modalPanelColor }}>
            <img src={teacher.avatar} alt="" className="mx-auto h-36 w-36 rounded-3xl border border-white/10 object-cover shadow-[0_22px_70px_rgba(0,0,0,0.35)]"/>
            ...
            <span className={cn("mt-6 inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-black uppercase tracking-[0.1em]", isActive
          ...
          <div className="grid gap-5">
            ...
            return (<section key={group.title} className="rounded-2xl border border-white/10 p-5" style={{ backgroundColor: modalPanelColor }}>
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-2">
                    {group.details.map((detail) => (<TeacherProfileDetail key={`${group.title}-${detail.label}`} detail={detail}/>))}
                  </div>
                </section>);
        })}
            ...
            icon: ShieldCheck,
              ...
              {isPasswordEditing ? (<form className="mt-5 grid gap-4" onSubmit={handlePasswordSubmit}>
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="grid gap-2">
                      <span className={modalLabelClass}>New Password</span>
                      <input className={modalInputClass} name="password" type="password" minLength={6} placeholder="New password" required/>
                    </label>
                    <label className="grid gap-2">
                      <span className={modalLabelClass}>Confirm Password</span>
                      <input className={modalInputClass} name="confirmPassword" type="password" minLength={6} placeholder="Confirm password" required/>
                    </label>
                  </div>
                  <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    <button type="button" onClick={() => setIsPasswordEditing(false)} disabled={isUpdatingPassword} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/12 px-5 text-sm font-black text-white/70 transition hover:border-white/30 hover:text-white disabled:cursor-not-allowed disabled:opacity-60">
                      Cancel
                    </button>
                    <button type="submit" disabled={isUpdatingPassword} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-cyanGlow px-5 text-sm font-black text-[#061014] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-70">
                      {isUpdatingPassword ? "Updating..." : "Save Password"}
                    </button>
                  </div>
                </form>) : (<button type="button" onClick={() => setIsPasswordEditing(true)} className="mt-5 inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-cyanGlow/35 px-5 text-sm font-black text-cyanGlow transition hover:bg-cyanGlow/10">
                  Set New Password
                </button>)}
            </section>
...
function TeacherProfileDetail({ detail, }) {
    const Icon = detail.icon;
    return (<div className="rounded-xl border border-white/10 bg-[#0b0310] p-4">
...
function ProfileStat({ label, value }) {
    return (<div className="rounded-xl border border-white/10 bg-white/[0.06] p-4">
...
function AddTeacherModal({ isSubmitting, onClose, onSubmit, }) {
    ...
    return createPortal(<div className="fixed inset-0 z-[9999] overflow-y-auto px-4 py-6 sm:px-6" style={{ backgroundColor: "#000000" }}>
      <div className="mx-auto max-w-5xl rounded-[1.35rem] border border-white/12 p-5 shadow-[0_32px_110px_rgba(0,0,0,0.55)] sm:p-7" style={{ backgroundColor: modalBackgroundColor }}>
        <div className="flex items-start justify-between gap-4 border-b border-white/10 pb-5">
          <div>
          ...
          <button type="button" onClick={onClose} disabled={isSubmitting} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 text-white/65 transition hover:border-[#f0b7ff]/45 hover:text-[#f0b7ff] disabled:cursor-not-allowed disabled:opacity-60" aria-label="Close add teacher form">
            <X size={21}/>
          </button>
        </div>
        ...
        <form className="mt-6 grid gap-6" onSubmit={handleSubmit}>
          <section className="rounded-2xl border border-white/10 p-5" style={{ backgroundColor: modalPanelColor }}>
            <div className="flex flex-col gap-5 lg:grid lg:grid-cols-[15rem_1fr]">
              <div className="grid content-start gap-3">
                ...
                <label className="group grid min-h-[13rem] cursor-pointer place-items-center rounded-2xl border border-dashed border-white/20 p-5 text-center transition hover:border-[#f0b7ff]/60" style={{ backgroundColor: modalFieldColor }}>
                  <span className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border border-white/15" style={{ backgroundColor: modalFieldColor }}>
                    {avatarPreviewDataUrl ? (<img src={avatarPreviewDataUrl} alt="" className="h-full w-full object-cover"/>) : (<ImagePlus className="text-[#f0b7ff]" size={32}/>)}
                  </span>
                  ...
                  <input ref={avatarInputRef} className="sr-only" type="file" accept="image/png,image/jpeg,image/webp" onChange={handleAvatarAttachment}/>
                </label>
                {avatarFileName && (<button type="button" onClick={clearAvatarAttachment} className="inline-flex w-fit items-center gap-2 text-sm font-black text-white/60 transition hover:text-[#f0b7ff]">
                    <X size={16}/>
                    Remove image
                  </button>)}
              </div>
          ...
          <section className="rounded-2xl border border-white/10 p-5" style={{ backgroundColor: modalPanelColor }}>
            <div className="grid gap-5 lg:grid-cols-3">
              ...
              <label className="grid gap-3 lg:col-span-2">
                ...
                <input className={modalInputClass} name="qualifications" placeholder="Dance diploma, certified instructor" required/>
              </label>
            </div>
            ...
            <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_21rem]">
              <label className="grid gap-3">
                ...
                <textarea className={`${modalInputClass} min-h-[14rem] resize-none leading-7`} name="biography" placeholder="Tell us about their journey, teaching style, and class experience..." required/>
              </label>
              ...
              <div className="grid content-start gap-5">
                <div className="grid gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {teachingDays.map((day) => (<button key={day} type="button" onClick={() => toggleTeachingDay(day)} className={cn("min-h-10 min-w-12 rounded-full border px-3 text-sm font-black transition", availableDays.includes(day)
                ? "border-[#f0b7ff] bg-[#f0b7ff] text-[#17061d]"
                ...
                <label className="grid gap-3">
                  ...
                  <span className="grid min-h-[8rem] cursor-pointer place-items-center rounded-2xl border border-dashed border-white/20 px-5 text-center transition hover:border-[#f0b7ff]/60" style={{ backgroundColor: modalFieldColor }}>
                    <span>
                    ...
                    <input className="sr-only" type="file" accept=".pdf,.doc,.docx" onChange={(event) => setSelectedFileName(event.target.files?.[0]?.name ?? "")}/>
                  </span>
                </label>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
            <button type="button" onClick={onClose} disabled={isSubmitting} className="inline-flex min-h-12 items-center justify-center rounded-lg border border-white/12 px-6 text-sm font-black text-white/70 transition hover:border-white/30 hover:text-white disabled:cursor-not-allowed disabled:opacity-60">
              Cancel
            </button>
            <button type="submit" disabled={isSubmitting} className="inline-flex min-h-12 items-center justify-center gap-3 rounded-lg bg-[#f0b7ff] px-7 text-sm font-black text-[#17061d] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-70">
